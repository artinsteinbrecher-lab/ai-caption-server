...
async def monitor_stdin():
    """监控标准输入，消费回车键"""
    # 后台进程没有交互式 stdin；避免 EOF 忙循环饿死服务任务。
...
async def main():
    ...
    def report_task_failure(task):
        ...
        if error is not None:
            logger.bind(tag=TAG).exception("后台服务任务启动失败: %s", error)

    ws_server = WebSocketServer(config)
    ws_task = asyncio.create_task(ws_server.start())
    ws_task.add_done_callback(report_task_failure)
    # 启动 Simple http 服务器
    ota_server = SimpleHttpServer(config)
    ota_task = asyncio.create_task(ota_server.start())
    ota_task.add_done_callback(report_task_failure)
    ...
    try:
        ...
        # 等待任务终止（必须加超时）
        cleanup_tasks = [ws_task, ota_task] if ota_task else [ws_task]
        if stdin_task:
            cleanup_tasks.insert(0, stdin_task)
        await asyncio.wait(
            cleanup_tasks,
            timeout=3.0,

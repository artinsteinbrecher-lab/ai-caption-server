...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        # continuous ASR idle guard
        self.idle_timeout_sec = int(config.get("idle_timeout_sec", 30))
    ...
    async def receive_audio(self, conn, audio, audio_have_voice):
        ...
        if self.asr_ws and self.is_processing and self.server_ready:
            if (not audio_have_voice and self.last_voice_time > 0
                    and now - self.last_voice_time > self.idle_timeout_sec):
                logger.bind(tag=TAG).info(
                    f"\u9759\u97f3\u8d85\u8fc7 {self.idle_timeout_sec} \u79d2，\u7ed3\u675f\u5f53\u524dASR\u4efb\u52a1（\u4e0b\u6b21\u8bf4\u8bdd\u81ea\u52a8\u91cd\u8fde）")
                await self._cleanup()
                return
            try:
    ...
    def _build_run_task_message(self) -> dict:
        ...
        # 只有当模型名称以v2结尾且vocabulary_id有值时才添加参数
                    ...
                    # 处理result-generated事件
                        ...
                        if conn.config.get("caption_mode", False) and text:
                            ...
                            if text != last_text or is_final:
                                seq = getattr(conn, "caption_seq", 0) + 1
                                conn.caption_seq = seq
                                await send_display_message(conn, text, caption={
                                    "action": "append" if is_final else "partial",
                                    "stream_id": self.task_id or "",
                                    "seq": seq,
                                })
                        ...
                        if is_final:
                            ...
                            if conn.config.get("caption_mode", False):
                                self.text = text
                                # continuous mode: keep this ASR WebSocket alive
                                conn.caption_last_text = ""
    ...
    async def _cleanup(self):
        ...
        if (self.forward_task and
            ...
            try:
                ...
                logger.bind(tag=TAG).debug(f"forward_task 取消时异常: {e}")
